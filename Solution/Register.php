...
if (isset($_POST["btnRegister"])) {
    ...
    if (!$validate) {
        ...
        if (!empty($firstNameError)) $errorArray["firstNameError"] = $firstNameError;


        $lastNameError = ValidateLastName($lastName);
        $customerInfo['lastName'] = $lastName;
        if (!empty($lastNameError)) $errorArray["lastNameError"] = $lastNameError;
        ...
        if (!empty($emailError)) $errorArray["emailError"] = $emailError;


        $phoneError = ValidatePhoneNumber($phone);
        $customerInfo['phone'] = $phone;
        if (!empty($phoneError)) $errorArray["phoneError"] = $phoneError;


        $passwordError = ValidatePassword($password);
        if (!empty($passwordError)) $errorArray["passError"] = $passwordError;
